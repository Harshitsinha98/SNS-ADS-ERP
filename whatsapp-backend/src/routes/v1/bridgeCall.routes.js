...
import { initiateHandler, availabilityHandler, answerHandler, answerConfirmHandler, answerNoInputHandler, customerAnswerHandler, customerAmdHandler, customerStatusHandler, confEventHandler, statusHandler, pollHandler, historyHandler, recordingsHandler } from "../../controllers/bridgeCall.controller.js";
...
export function createBridgeCallRoutes() {
  ...
  router.get("/availability", requireAuth, availabilityHandler);
  router.post("/poll", requireAuth, pollHandler);
  router.get("/history", requireAuth, historyHandler);
  router.get("/recordings", requireAuth, recordingsHandler);
  ...
  router.post("/customer-amd", plivoLimiter, customerAmdHandler);
  router.get("/customer-amd", plivoLimiter, customerAmdHandler);
